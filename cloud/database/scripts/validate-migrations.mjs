...
for (const { name, text } of migrations) {
  ...
  if (!match) { fail(`${name}: migration filename must be 0001_descriptive_name.sql.`); continue; }
  const version = Number.parseInt(match[1], 10);
  if (version !== expected) fail(`${name}: expected migration prefix ${String(expected).padStart(4, '0')}.`);
  expected += 1;
  ...
  for (const table of createTables) {
    ...
    if (!/PRIMARY KEY/i.test(tableSql)) fail(`${name}: table ${tableName} must declare a primary key.`);
  }
}

const framework = migrations.find((migration) => migration.name.startsWith('0001_'))?.text ?? '';
if (!/CREATE TABLE IF NOT EXISTS schema_migrations/i.test(framework)) fail('0001 migration must create schema_migrations.');
if (!/CREATE TABLE IF NOT EXISTS seed_runs/i.test(framework)) fail('0001 migration must create seed_runs.');
...
for (const required of ['companies', 'rooms', 'devices', 'device_credentials', 'pairing_sessions', 'room_configurations', 'device_commands', 'device_events', 'audit_events', 'releases']) {
  if (!new RegExp(`CREATE TABLE(?: IF NOT EXISTS)?\\s+${required}\\b`, 'i').test(allMigrationSql)) fail(`Required table ${required} is missing from migrations.`);
}
if (/raw_code|pairing_code\s+text/i.test(allMigrationSql)) fail('Pairing migrations must not store raw pairing codes. Store code_hash instead.');

const seeds = readSqlFiles(seedsDir);
for (const { name, text } of seeds) {
  if (!/\bBEGIN\s*;/i.test(text) || !/\bCOMMIT\s*;/i.test(text)) fail(`${name}: seed must use an explicit transaction.`);
  if (!/ON CONFLICT/i.test(text)) fail(`${name}: seed must be idempotent with ON CONFLICT.`);
}
