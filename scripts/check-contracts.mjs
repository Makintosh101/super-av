...
const patterns = {
  messageId: new RegExp(`^msg_${ulid}$`),
  deviceId: new RegExp(`^dev_${ulid}$`),
  connectionId: new RegExp(`^conn_${ulid}$`),
  commandId: new RegExp(`^cmd_${ulid}$`),
  utc: /Z$/
};
...
for (const t of wsTypes) {
  ...
  if (bad) {
    const save = [...fail]; fail.length = 0; validateWs(bad); const rejected = fail.length > 0; fail.length = 0; fail.push(...save);
